...
export async function notificationRoutes(app:FastifyInstance){
 app.get('/notifications', {preHandler:requireAuth}, async(req:any)=>db.query(`SELECT id,event_type,channel,title_key,body_key,payload,status,created_at,sent_at FROM notifications WHERE user_id=$1 UNION ALL SELECT id,type AS event_type,'in_app' AS channel,title AS title_key,body AS body_key,jsonb_build_object('reportId',report_id) AS payload,CASE WHEN read_at IS NULL THEN 'PENDING' ELSE 'READ' END AS status,created_at,NULL::timestamptz AS sent_at FROM report_notifications WHERE user_id=$1 ORDER BY created_at DESC LIMIT 100`,[req.user.id]).then(r=>r.rows));
